...
class MHSA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_attn: bool = False) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        if return_attn:
            # Return attention before dropout: (B, num_heads, N, N)
            return x, attn
        return x
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, return_attn: bool = False):
        ...
        if return_attn:
            token_out, attn = self.token_mixer(normed_x, return_attn=True)
            if self.use_layer_scale:
                x = x + self.drop_path(self.layer_scale_1 * token_out)
            else:
                x = x + self.drop_path(token_out)
            x = x + self.drop_path(self.layer_scale_2 * self.convffn(x) if self.use_layer_scale else self.convffn(x))
            return x, attn
        else:
            if self.use_layer_scale:
                x = x + self.drop_path(self.layer_scale_1 * self.token_mixer(normed_x))
                x = x + self.drop_path(self.layer_scale_2 * self.convffn(x))
            else:
                x = x + self.drop_path(self.token_mixer(normed_x))
                x = x + self.drop_path(self.convffn(x))
            return x
...
class FastViT(nn.Module):
    ...
    def forward_tokens(self, x: torch.Tensor, return_attn: bool = False, *args, **kwargs) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        last_attn = None
        for idx, block in enumerate(self.network):
            # Check if this is an AttentionBlock (last stage has attention blocks)
            if return_attn and isinstance(block, nn.Sequential):
                # Check if the last block in the sequence is an AttentionBlock
                for sub_block in reversed(block):
                    if isinstance(sub_block, AttentionBlock):
                        x, last_attn = sub_block(x, return_attn=True)
                        break
                else:
                    x = block(x)
            elif return_attn and isinstance(block, AttentionBlock):
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> Union[Tensor, Dict[str, Tensor]]:
        ...
        # through backbone
        return_attn = kwargs.get("return_attention_maps", False)
        if return_attn:
            x, attention_maps = self.forward_tokens(x, return_attn=True)
        else:
        ...
        if kwargs.get("return_image_embeddings", False):
            ...
            if return_attn and attention_maps is not None:
                out_dict.update({"attention_maps": attention_maps})
            return out_dict
        else:
            if return_attn and attention_maps is not None:
                return {"logits": cls_out, "attention_maps": attention_maps}
            return cls_out
