...
class MobileCLIPVisionTower(nn.Module):
    ...
    def forward_images(self, images, return_attention_maps=False):
        if type(images) is list:
            ...
            for image in images:
                ...
                if return_attention_maps and "attention_maps" in image_forward_out:
                    attention_maps_list.append(image_forward_out["attention_maps"])
        else:
            ...
            if return_attention_maps and "attention_maps" in image_forward_outs:
                attention_maps_list = image_forward_outs["attention_maps"]

        if return_attention_maps:
            if type(images) is list:
                return image_features, attention_maps_list[0] if attention_maps_list else None
            else:
                return image_features, attention_maps_list if return_attention_maps else None
        return image_features
