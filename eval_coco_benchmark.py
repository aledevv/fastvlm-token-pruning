...
def run_inference_single(model_data, image_path, args, pruning_method='none', keep_ratio=1.0):
    ...
    attention_mask = (input_ids != tokenizer.pad_token_id).long().to(device) # Fix for warning
    ...
    # Calcola n_orig e n_pruned per l'output, indipendentemente dalla modalità
    # Nota: se non è prunato, n_pruned == n_orig
    ...
    if n_orig is None or n_pruned is None:
        # Se non è prunato, dobbiamo ricalcolare per avere i valori
        with torch.inference_mode():
...
def evaluate_coco(args):
    ...
    # Progress bar principale (sulle immagini)
    ...
    df.to_csv(os.path.join(args.output_dir, 'coco_results.csv'), index=False)
    print(f"\n💾 Risultati salvati in {args.output_dir}/coco_results.csv")
...
def plot_results(df, out_dir):
    ...
    - df: pandas.DataFrame con colonne almeno ['method','ratio','sim_vs_gt','speedup']
    - out_dir: directory dove salvare i png
    """
    ...
    # Consistent palette requested by user
    palette = {"attention": "blue", "similarity": "orange", "norm": "green"}

    # 1. Original Similarity vs Retention (Accuracy)
    ...
    # 1b. Unified Accuracy vs Ratio (0 -> slightly above max)
    plt.figure(figsize=(10, 6))
    ax = plt.gca()
    sns.lineplot(data=df_methods, x='ratio', y='sim_vs_gt', hue='method', style='method', markers=True, dashes=False, palette=palette, ax=ax)
    if baseline_acc is not None:
        ax.axhline(y=baseline_acc, color='black', linestyle='--', label='Baseline Accuracy')
    ax.set_title('Accuracy vs Token Retention Ratio (Unified Scale)')
    ax.set_xlabel('Retention Ratio (Lower is more pruning)')
    ax.set_ylabel('Similarity with Ground Truth')
    # set y from 0 to a little above max similarity
    if not df_methods['sim_vs_gt'].isnull().all():
        ymax = df_methods['sim_vs_gt'].max()
        ax.set_ylim(0, ymax * 1.05 if ymax > 0 else 1.0)
    ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, 'accuracy_vs_ratio_unified.png'))
    plt.close()

    # 2. Pareto Frontier: Speedup vs Accuracy (with labels next to marks)
    plt.figure(figsize=(10, 6))
    ax = plt.gca()
    ...
    # 3. New Pareto: average over experiments per method and ratio
    df_avg = df_methods.groupby(['method', 'ratio'], as_index=False).agg({'speedup':'mean', 'sim_vs_gt':'mean'})

    plt.figure(figsize=(10, 6))
    ax2 = plt.gca()
    sns.scatterplot(data=df_avg, x='speedup', y='sim_vs_gt', hue='method', s=150, ax=ax2, palette=palette)
    if baseline_acc is not None:
        ax2.scatter([1.0], [baseline_acc], color='black', s=150, label='Baseline', marker='X')
    # annotate averaged points
    for _, row in df_avg.iterrows():
        ...
        if x is None or y is None or pd.isna(x) or pd.isna(y):
            continue
        label = f"{row['method']} ({row['ratio']})"
        ax2.annotate(label, (x, y), textcoords="offset points", xytext=(5,5), ha='left', fontsize=9)

    ax2.set_title('Trade-off (Averaged): Speedup vs Accuracy per Method & Ratio')
    ax2.set_xlabel('Avg Speedup Factor (x)')
    ax2.set_ylabel('Avg Similarity with Ground Truth')
    ax2.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, 'tradeoff_pareto_avg.png'))
    plt.close()
